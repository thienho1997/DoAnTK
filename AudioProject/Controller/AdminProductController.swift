...
//  AudioProject
//
//  Created by HoThienHo on 6/15/19.
//  Copyright © 2019 hothienho. All rights reserved.
//

import UIKit
import Firebase
class AdminProductController: UITableViewController {
...
//    }()
    ...
    lazy var viewForCart: UIView = {
        ...
        let labelNumber = UILabel()
        labelNumber.backgroundColor = #colorLiteral(red: 0.5568627715, green: 0.3529411852, blue: 0.9686274529, alpha: 1)
        labelNumber.layer.cornerRadius = 7
        labelNumber.clipsToBounds = true
        labelNumber.text = "1"
        labelNumber.textAlignment = .center
        labelNumber.textColor = #colorLiteral(red: 1.0, green: 1.0, blue: 1.0, alpha: 1.0)
        labelNumber.font = UIFont(name: labelNumber.font.fontName, size: 13)
        ...
        labelNumber.topAnchor.constraint(equalTo: v.topAnchor).isActive = true
        labelNumber.rightAnchor.constraint(equalTo: v.rightAnchor).isActive = true
        labelNumber.heightAnchor.constraint(equalToConstant: 14).isActive = true
        labelNumber.widthAnchor.constraint(equalToConstant: 14).isActive = true
        return v
    }()
    @objc func cartTap(){
    ...
    @objc func buttonAddAction(){
        let adminAddProductController = AdminAddProductController()
        adminAddProductController.menu = menu
        self.navigationController?.pushViewController(adminAddProductController, animated: true)
    }
    override func viewDidLoad() {
       ...
       // self.navigationItem.rightBarButtonItem = buttonItem
        ...
        self.tableView.tableFooterView = containView
    ...
    override func tableView(_ tableView: UITableView, editActionsForRowAt indexPath: IndexPath) -> [UITableViewRowAction]? {
        let editButton = UITableViewRowAction(style: UITableViewRowAction.Style.normal, title: "Edit") { (rowAction, indexPath) in
            let adminEditController = AdminAddProductController()
            adminEditController.adminProductController = self
            adminEditController.product = self.products[indexPath.row]
            adminEditController.menu = self.menu
        ...
        let deleteButton = UITableViewRowAction(style: UITableViewRowAction.Style.normal, title: "Delete") { (rowAction, indexPath) in
            ...
            let databaseRef = Database.database().reference().child("products").child(self.menu!.id!).child(product.id!)
